...
- A `SeedFingerprint::adopt()` escape was tried — let a product-page view stamp on the seeder's behalf once it had proved the data matched. **Removed again after an adversarial review, and Fix 3 made it unnecessary.** The proof could not be written honestly: a seeder does not only insert, it also prunes retired courses, repairs drifted category slugs in place and clears empty categories. Any cheap check looks at rows that *exist*, so on a populated production database it passes for a half-killed run — and stamping then cancels those repair passes **permanently**, where an unstamped fingerprint just retries every five minutes until it succeeds. A test now asserts that serving a product page never stamps anything.
