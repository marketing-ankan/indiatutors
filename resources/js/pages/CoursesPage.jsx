      ...
      <div className="grid lg:grid-cols-[260px_1fr] gap-8">
        <aside className="space-y-5">
          ...
          <div>
            ...
            <ul className="space-y-1 text-sm">
              ...
              {categories.map(c=>(
                <li key={c.id}><button onClick={()=>setParam('category',c.slug)} className={`w-full text-left px-2 py-1.5 rounded flex justify-between items-center ${category===c.slug?'bg-brand-50 text-brand-700 font-semibold':'text-slate-700 hover:bg-slate-50'}`}><span>{c.name}</span><span className="text-xs text-slate-400">{c.course_count??0}</span></button></li>
              ))}
            </ul>
          </div>
        </aside>
        <div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-20 text-slate-500">No courses found. Try clearing filters.</div>
          )}
        </div>
      </div>
    </div>
